...
import { s280_DeleteTalkUser } from "../../service/S280_DeleteTalkUser"
...
export const SC999_V12_DeleteTalk = () => {
    ...
    // --------------------------------------------------------------
    const deleteTalk = async (talkInfo: SC999_TalkUserInfo) => {
        ...
        if (check(talkInfo)) {
            ...
            // サービスを実行する
            await s280_DeleteTalkUser(talkId, userId)
    ...
    return (
        <>
            ...
            <Text>{"\n"}</Text>
            <Button size="sm" style={SC999_Style.regularBtn} onPress={() => { deleteTalk(talkInfo); }}>トーク情報削除</Button>
        </>
    )
}
