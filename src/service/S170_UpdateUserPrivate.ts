...
const SERVICE_ID = "S170"

export const s170_UpdateUserPrivate = async (
    userId: string,
    loginId: string,
    password: string,
    logUserId: string
    ...
    // Firebase処理
    const result_FB = await updateDoc(doc(DB_FIREBASE, FIREBASE_COLLECTIONS.M051_UserPrivate, docId), newUserPrivateInfo);
    // 戻り値を定義
    const resultObj = {
        result_FB: result_FB
    }
